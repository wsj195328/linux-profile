...
install_go(){
	...
	if [ $(getconf WORD_BIT) = '32' ] && [ $(getconf LONG_BIT) = '64' ];then
		# 判断文件是否已经存在
		if [ ! -f $SELFPATH/go1.8.3.linux-amd64.tar.gz ];then
			wget https://www.golangtc.com/static/go/1.8.3/go1.8.3.linux-amd64.tar.gz
		fi
	    tar zxvf go1.8.3.linux-amd64.tar.gz
	else
		if [ ! -f $SELFPATH/go1.8.3.linux-386.tar.gz ];then
			wget https://www.golangtc.com/static/go/1.8.3/go1.8.3.linux-386.tar.gz
		fi
	    tar zxvf go1.8.3.linux-386.tar.gz
	fi
	mv go /usr/local/
